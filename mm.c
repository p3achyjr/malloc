...
void mm_checkheap(int lineno) {
  ...
  for (bp = heap_listp; GET_SIZE(HDRP(bp)) > 0; 
       bp = NEXT_BLKP(bp)) {
    // header and footer equal
    if(HDRP(bp) != FTRP(bp)) {
      printf("checkheap called from lineno %d\n", lineno);
      printf("error: block %lx has different header and footer\n", 
             (unsigned long)bp);
    }

    // // block is of specified length
    // blkSize = GET_SIZE(HDRP(bp));
    // if(HDRP(bp) != GET(bp + (blkSize-2) * 4)) {
    //   printf("error: block")
    // }
  }
}
